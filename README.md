...
## Hardware
 SDIO is forced/limited to 3.3V signalling, to keep the BOM simple. The project is geared towards ICs like GL823K (Genesys Logic) that cost <$0.40 in single qty. This initial code is tested with a generic USB3.0 card reader, with long jumper wires connecting the SDIO pins to the Pico 2. Hence only the default speed mode (25 MHz SD_CLK) has been tested here. Pico 2 is clocked at 250 MHz. The card reader was connected to the Pico 2 as follows:
...
* Test the boost mode as claimed by the IC. I guess they included it to saturate USB HS speeds, without the complications of 1.8V voltage switch.
...
* USB 3.0 card readers too are quite cheap, and SD104 mode would allow for ~90 MB/s but that involves 1.8V signalling (possible with level shifters) and the need to sample a 104/208 MHz clock with a 250 MHz sysclk (unlikely). Most resources mention this mode uses dynamic phase alignment which helps with Card -> Host but I'm not sure it would do anything for Host -> Card?